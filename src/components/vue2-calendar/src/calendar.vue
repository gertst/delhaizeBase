...
export default {
    ...
    shownIndicator: {
      get() {
        ...
        if (this.indicator.detail) {
          ...
          indicator.detail = this.startDate.getMonth() + 1 + ''
        }
        return indicator
      }
    },
  ...
  methods: {
    ...
    prevView() {
      if (this.view === 'month') {
        this.p_startDate = new Date(this.startDate.getFullYear(), this.startDate.getMonth() - 1, this.startDate.getDate())
      } else {
        this.p_startDate = new Date(this.startDate.getFullYear(), this.startDate.getMonth(), this.startDate.getDate() - 7)
      }
      this.$nextTick(() => {
        this.$emit('prev')
      })
