<!--https://github.com/ridewn/vue-date-picker/blob/master/src/DatePicker.vue-->
					...
					<table>
						...
						<tr v-for="days in calendar">
							<td :style="{
							    ...
							    v-for="day in days"
							    tabindex="0"
							    :redrawId="redrawId" :activities="activities.length"
...
<script>
	...
	export default {
		...
		computed: {
			...
			calendar () {
				...
				const dayObjects = days.map((day) => {
					...
					const activity = this.activity(day);

					return { day, currentDay, selected, disabled, activity }
				})
		...
		created () {
			...
			this.$root.$on("MONTH_ACTIVITIES_UPDATED", (payload) => {
				this.activities = payload;
				this.redrawId = Math.random();
				this.setDate(this.redrawId);
			});
		},
